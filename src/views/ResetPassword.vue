        ...
        <main class="auth-card" role="main" aria-labelledby="resetTitle">
          ...
          <section v-if="tokenMissing" class="cardc-form">
            <button
              class="btn-teal"
              type="button"
              @click="$router.replace('/findpassword')"
            >
              비밀번호 찾기 화면으로 이동
            </button>
          </section>
          ...
          <form v-else class="cardc-form" @submit.prevent="onSubmit" novalidate>
            ...
            <div
              v-if="
                error && (error.includes('expired') || error.includes('token'))
              "
              class="mt16"
            >
              <button
            ...
            <p v-if="done" class="reset-done-msg" aria-live="polite">
              <span class="done-main"
                >비밀번호가 성공적으로 변경되었습니다.</span
              >
              <span class="done-sub">이제 로그인할 수 있어요.</span>
              <router-link to="/login" class="done-login-link"
                >로그인 하러가기</router-link
              >
            </p>
          </form>
        </main>
